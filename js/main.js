(function start() {
    ...
    var trailMaxLen = 55;
    Orbital.trails = Orbital.planets
        .map(function (body) {
    ...
    var cometMesh = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({ color: 0xf0ead8, transparent: true, opacity: 0 })
    );
    ...
    cometTrailGeo.setAttribute("position", new THREE.BufferAttribute(cometTrailBuf, 3));
    cometTrailGeo.setDrawRange(0, 0);
    var cometTrailMat = new THREE.LineBasicMaterial({ color: 0xe8dfc8, transparent: true, opacity: 0 });
    Orbital.scene.add(new THREE.Line(cometTrailGeo, cometTrailMat));
    var comet = {
        active: false, progress: 0, duration: 5,
        start: new THREE.Vector3(), end: new THREE.Vector3(),
        history: [], nextSpawn: 22
    };
    function spawnComet() {
        var a = Math.random() * Math.PI * 2;
        var r = 44;
        comet.start.set(Math.cos(a) * r, Orbital.systemCenterY + (Math.random() - 0.5) * 22, Math.sin(a) * r);
        var b = a + Math.PI + (Math.random() - 0.5) * 1.0;
        comet.end.set(Math.cos(b) * r, Orbital.systemCenterY + (Math.random() - 0.5) * 22, Math.sin(b) * r);
        comet.progress = 0;
        comet.history = [];
    ...
    Orbital.spawnRipple = function (planet) {
        ...
        var mat = new THREE.MeshBasicMaterial({ color: 0xd8d4cc, transparent: true, opacity: 0.65, side: THREE.DoubleSide });
    ...
    function tick() {
        ...
        if (!Orbital.isOrbitPaused) {
            orbitTime += delta;
            Orbital.updatePlanets(orbitTime);
            if (Orbital.asteroidBelt) { Orbital.asteroidBelt.rotation.y += delta * 0.022; }
        }
        ...
        Orbital.bodyLabels.forEach(function (item) {
            ...
            labelVec.set(item.mesh.position.x, item.mesh.position.y + (item.mesh.userData.labelYOffset || 2), item.mesh.position.z);
            labelVec.project(Orbital.camera);
        ...
        if (!reducedMotion && !comet.active && comet.nextSpawn <= 0 && !Orbital.focusedPlanet) { spawnComet(); }
        if (comet.active) {
            ...
            if (comet.history.length > cometTrailLen) { comet.history.pop(); }
