...
The hackathon version had a Python FastAPI server. For v1.0 I rewrote the engine in Rust with Axum and Tokio. The server runs a 60Hz authoritative game loop, and Python's GIL plus the Pydantic and NumPy overhead in that hot path was the bottleneck. I did a full rewrite instead of a PyO3 shim because a clean break was simpler than a hybrid. The wire protocol stayed the same, so none of the TypeScript clients had to change.

Games are Rust plugins behind a trait. Boxing was built as the first plugin instead of into the engine, which forced the engine and game boundary to be real. Dance and FPS boxing were added later without changing the engine core.
