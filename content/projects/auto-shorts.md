...
For Reddit posts, Claude rewrites the post into a script that fits in a short video and doesn't sound weird when it's read out loud. Tweets don't need a script, since it just reads the tweet verbatim. ElevenLabs generates the audio overlay and gives back timestamps for each word, which is how the subtitles stay synced. Then FFmpeg renders a 1080x1920 video. Reddit stories get gameplay footage in the background, and tweets get a screenshot of the tweet rendered with Playwright.
...
Got over 1m views from uploading these videos but decided to stop uploading and work on something with more "social impact"
